...
if strfind(raydirection,'x')
  ...
  gridOUTPUT(:,:,:,countdirections) = permute( VOXELISEinternal(gridCOy,gridCOz,gridCOx,meshXYZ(:,[2,3,1],:),'x') ,[3,1,2] );
end

if strfind(raydirection,'y')
  countdirections = countdirections + 1;
  gridOUTPUT(:,:,:,countdirections) = permute( VOXELISEinternal(gridCOz,gridCOx,gridCOy,meshXYZ(:,[3,1,2],:),'y') ,[2,3,1] );
end

if strfind(raydirection,'z')
  countdirections = countdirections + 1;
  gridOUTPUT(:,:,:,countdirections) = VOXELISEinternal(gridCOx,gridCOy,gridCOz,meshXYZ,'z');
end
...
%==========================================================================
function [gridOUTPUT] = VOXELISEinternal(gridCOx,gridCOy,gridCOz,meshXYZ,raydirn)
...
for loopY = meshYminp:meshYmaxp
  for loopX = meshXminp:meshXmaxp
    ...
    possibleCROSSLIST = find( meshXYZmin(:,1) <= gridCOx(loopX) );
    possibleCROSSLIST = possibleCROSSLIST( meshXYZmax(possibleCROSSLIST,1) >= gridCOx(loopX) );
    possibleCROSSLIST = possibleCROSSLIST( meshXYZmin(possibleCROSSLIST,2) <= gridCOy(loopY) );
    possibleCROSSLIST = possibleCROSSLIST( meshXYZmax(possibleCROSSLIST,2) >= gridCOy(loopY) );
    ...
    if isempty(possibleCROSSLIST)==0  %Only continue the analysis if some nearby facets were actually identified
      ...
      for loopCHECKFACET = possibleCROSSLIST'
        ...
        checkvertexcrossing = (meshXYZ(loopCHECKFACET,1,:)==gridCOx(loopX)) + (meshXYZ(loopCHECKFACET,2,:)==gridCOy(loopY));
        
        if max(checkvertexcrossing)<2  %Ray does not cross any of the vertices of the facet
          ...
          if ( testV1==1 && testV2==1 && testV3==1 ) || sum([testV1edge,testV2edge,testV3edge])==1
            facetCROSSLIST = [facetCROSSLIST,loopCHECKFACET];
          end %if

        elseif max(checkvertexcrossing)==2  &&  sum(checkvertexcrossing==2)==1 % Ray crosses at one vertex of the facet only

          vertexcrossed  = find( meshXYZ(loopCHECKFACET,1,:)==gridCOx(loopX) & meshXYZ(loopCHECKFACET,2,:)==gridCOy(loopY) );
          vertexCOzCROSS = [vertexCOzCROSS,meshXYZ(loopCHECKFACET,3,vertexcrossed)];
...
if countCORRECTIONLIST>0
  ...
  for loopC = 1:countCORRECTIONLIST
    ...
    voxelsforcorrection = find(voxelsforcorrection>=4);
...
%disp([' Ray tracing in ',raydirn,' direction: ',num2str(countCORRECTIONLIST),' rays (',num2str(countCORRECTIONLIST/(voxcountX*voxcountY)*100,'%5.1f'),'% of all rays) exactly crossed a facet edge and had to be computed by interpolation.'])
